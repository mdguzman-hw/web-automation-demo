...
def test_bat_web_024(sentio_provider, quantum):
    sentio_provider.driver.get(sentio_provider.base_url)
    assert quantum.base_url in sentio_provider.current_url.lower()


# TEST: Sentio Beta Provider Login
def test_bat_web_025(sentio_provider, quantum, credentials):
...
def test_bat_web_027(sentio_provider, quantum):
    assert sentio_provider._is_authenticated
